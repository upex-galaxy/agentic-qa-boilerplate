...
3. Records the result (PASS/FAIL, duration, error) to in-memory Map AND NDJSON file
...
```
File: reports/atc_results.ndjson
...
|-------|-------------|------|
| Test execution | `storeResult()` appends one line per ATC call | `reports/atc_results.ndjson` (created/appended) |
| Global teardown | `generateAtcReport()` reads all lines, aggregates by testId | `reports/atc_results.json` (created) |
| After report | NDJSON file is deleted (cleanup) | `reports/atc_results.ndjson` (deleted) |
...
| File | Responsibility |
|------|---------------|
| `tests/utils/decorators.ts` | `@atc`, `@step`, `formatArgs`, NDJSON write, `generateAtcReport()` |
| `tests/teardown/global.teardown.ts` | Calls `generateAtcReport()` + `syncResults()` |
| `tests/utils/jiraSync.ts` | TMS API integration (Xray Cloud, Jira Direct) |
| `tests/KataReporter.ts` | Terminal output (captures `test.step` events) |
| `playwright.config.ts` | Reporter chain, project/teardown configuration |
| `config/variables.ts` | TMS config values (`config.tms.*`) |
| `reports/atc_results.ndjson` | Ephemeral — exists only during test run |
...
**Cause**: NDJSON file doesn't exist or is empty when teardown reads it.

**Check**:
1. Verify `@atc` decorators are applied (not just `@step`)
2. Check that `reports/atc_results.ndjson` is created during test execution
3. Ensure teardown runs AFTER test projects (check `playwright.config.ts` dependencies)
...
3. Check that `generateAtcReport()` ran before `syncResults()`

---

**Source files**: `tests/utils/decorators.ts`, `tests/utils/jiraSync.ts`, `tests/teardown/global.teardown.ts`
