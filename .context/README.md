# .context/ - Context Engineering Directory

This directory contains all the documentation that the AI reads to work on the project.

## Structure

```
.context/
├── business/                            # All business-level context (single home)
│   ├── business-model.md               #   /project-discovery (Phase 1 — Constitution)
│   ├── domain-glossary.md              #   /project-discovery (Phase 1 — Constitution)
│   ├── business-data-map.md            #   project-context mode data
│   ├── business-feature-map.md         #   project-context mode features
│   └── business-api-map.md             #   project-context mode api
├── PRD/                PHASE 2: Architecture - Product Requirements
├── SRS/                PHASE 2: Architecture - Software Requirements
├── ADR/                Architecture Decision Records — test architecture (append-only)
├── PBI/                Gitignored Jira cache — rebuilt by `bun run context:hydrate`
├── reports/            Sprint-level testing frameworks (managed by /sprint-testing)
└── master-test-plan.md                 # project-context mode test-plan — what to test and why
```

Workflow guidelines live inside the skills in `.agents/skills/`, shared by every supported harness. Each skill bundles its own `references/` material.

## Getting Started

### Project Memory Setup

Load the `/project-discovery` skill in your AI assistant. It:
1. Detects your AI tool and generates the correct instruction file (`AGENTS.md`, plus any harness shim such as `CLAUDE.md`)
2. Generates a professional `README.md`
3. Generates the `.context/` artifacts listed above

### Project Phases

**Discovery (one-time, handled end-to-end by `/project-discovery`):**

- Business constitution -> generates `business/business-model.md` and `business/domain-glossary.md`
- Architecture -> generates `PRD/` and `SRS/`
- Infrastructure -> complements `SRS/`
- Specification -> `PBI/` is NOT generated here: it is a gitignored Jira cache, rebuilt by `bun run context:hydrate`

**QA workflow (iterative, via skills):**

- `/sprint-testing` -- in-sprint planning, execution, and reporting per ticket
- `/test-documentation` -- TMS documentation and prioritization
- `/test-automation` -- KATA test planning + coding + review
- `/regression-testing` -- regression execution and GO/NO-GO

## Skills by Role

| Role          | Primary Skills                                         |
| ------------- | ------------------------------------------------------ |
| QA Engineer   | `/sprint-testing`, `/test-documentation`               |
| QA Automation | `/test-automation`, `/regression-testing`              |
| Any role      | `AGENTS.md` "MCPs (decision rules)" section for MCPs   |

## Recommended Context Files

These files are generated by the project's discovery skill and by the `project-context` skill. If they don't exist yet, create them with the generator listed below:

| File | Generator | Required For |
|------|-----------|-------------|
| `business/business-data-map.md` | `project-context` mode `data` | Understanding system flows and entities |
| `business/business-feature-map.md` | `project-context` mode `features` | Feature catalog, CRUD matrix, feature flags |
| `business/business-api-map.md` | `project-context` mode `api` | Auth model, critical journey endpoints, architecture behind the API |
| `master-test-plan.md` | `project-context` mode `test-plan` | Knowing what to test and why (risk-ranked) |
| `business/business-model.md` | `/project-discovery` (Phase 1 — Constitution) | Business context for test planning |
| `business/domain-glossary.md` | `/project-discovery` (Phase 1 — Constitution) | Consistent terminology |
| `PRD/*.md` | `/project-discovery` (Phase 2 — Architecture) | User personas, user journeys |
| `SRS/*.md` | `/project-discovery` (Phase 2 + Phase 3 — Infrastructure) | Technical specs, API contracts |
| `ADR/*.md` | Human QA architect, or `/project-discovery` (SRS/infra) · `/framework-development` · `/sprint-testing` — AI drafts, human approves | **Exception: append-only, never regenerated.** Test-architecture decisions (runner, fixtures, isolation, flake policy); superseded by a newer ADR, never overwritten. See `ADR/README.md` |
| `api/schemas/` | `bun run api:sync` | OpenAPI-derived TypeScript types for integration tests |

**Minimum viable context:** `business/business-data-map.md` + `master-test-plan.md` (run modes `data` and `test-plan` first).

---

## References

- **Project Memory**: `AGENTS.md` (Claude Code reads it through the generated `CLAUDE.md` shim)
- **Context Engineering**: `../CONTEXT.md`
- **Workflow skills**: `.agents/skills/` (each skill self-describes via its SKILL.md)

---

**Last Updated**: 2026-04-03
