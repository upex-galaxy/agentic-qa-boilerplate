...
| `{{jira.<slug>}}` | **Jira custom field reference** — portable pointer to a Jira custom field. | `.agents/jira-required.yaml` (canonical declaration of expected fields) AND `.agents/jira-fields.json` (workspace-resolved IDs). Skills never hardcode `customfield_XXXXX`. | `bun run lint:agents` (slug must be declared in the manifest) AND `bun run jira:check` (slug must resolve to a real field in `jira-fields.json`). |
...
| `bun run jira:sync-issues` | Pull Jira Epics/Stories into `.context/PBI/` markdown files (content sync, not metadata). Subcommands: `status`, `pull` (with `--epic`, `--story`, `--include-comments`, `--dry-run`, `--json`). |
...
| `bun run lint:agents` | Validate every `{{VAR}}`, `{{jira.<slug>}}`, `{{jira.<slug>.<option>}}`, `{{jira.work_type.*}}`, `{{jira.status.*}}` and `{{jira.transition.*}}` reference across `.claude/`, `templates/`, `.context/`, `CLAUDE.md`. Exits 1 if any are undeclared. |
