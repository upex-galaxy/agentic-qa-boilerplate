...
> **Companion references (QA-side)**: `sprint-testing/references/reporting-templates.md`, `sprint-testing/references/acceptance-test-planning.md`, `test-documentation/references/jira-setup.md`, `test-documentation/references/jira-test-management.md`, `test-documentation/references/tms-architecture.md`, `shift-left-testing/references/atp-outline-template.md`, `acli/references/gotchas.md` (covers acli CLI usage gotchas — different surface).
...
- **Permanent fix**: the converter at `acli/scripts/md-to-adf.ts` strips `strong` / `em` marks when an inline `code` mark co-occurs on the same text node, so Jira REST `POST /rest/api/3/issue` accepts documents that would otherwise trigger HTTP 400 `INVALID_INPUT`. Fix lives inside the `acli` skill — do not duplicate the patch here.
...
This file is cited from `sprint-testing/references/reporting-templates.md` (ATR + bug evidence rich-text writes), `sprint-testing/references/acceptance-test-planning.md` (ATP body publish to Story), `test-documentation/references/jira-setup.md` (custom-field configuration + initial body writes), `test-documentation/references/jira-test-management.md` (Modality jira-native Test issue body writes), `test-documentation/references/tms-architecture.md` (Test / TestPlan / TestExecution body writes), and `shift-left-testing/references/atp-outline-template.md` (pre-sprint ATP body publish during refinement). Any workflow that publishes rich-text content to Jira must link here for the two gotchas above before issuing its first write.
