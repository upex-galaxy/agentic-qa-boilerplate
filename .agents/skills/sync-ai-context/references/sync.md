...
**Rule 4 — High inbound reference density** (≥ 3 cross-references from canonical skills): scan `.agents/skills/**/*.md` for file-path mentions, count per target, flag those with ≥ 3 references.
...
| `supplementary` | Referenced by skills but not always loaded | Patched for command/path/name drift; other prose preserved |
| `index` | `.context/` outputs of `project-context` modes | NOT rewritten — existence only checked; AI memory "Context System" section updated to reflect reality |
...
**Extendability note for future maintainers**: to add a new document to the sync scope, either (a) ensure it matches one of the 5 rules above, or (b) add it explicitly to the "Sync scope" table in this file's header. Both approaches are equivalent — the audit picks up rule-matches automatically.
...
- Skill, mode, or legacy name changed (e.g. `/refresh-ai-memory` → `sync-ai-context`)
...
- Skill table has a stale description
...
- §5 SKILLS + MODES + MCPs REGISTRY (its tables — patched for skill name changes via Step 4.5; updated manually when the skill set evolves)
...
Renamed on purpose, never structural drift (the command-alias layer is retired): `## 5. SKILLS + COMMANDS + MCPs REGISTRY` → `## 5. SKILLS + MODES + MCPs REGISTRY`, and `### Compatibility command aliases` → `### Skill modes`.
...
2. Identify drifted user-facing copy by cross-referencing the canonical sources gathered in Step 2 (`package.json` scripts, skill names and their modes from `.agents/skills/`, paths from `AGENTS.md`'s Context Loading Map, environment URLs from `.agents/project.yaml`).
...
This step focuses on `AGENTS.md`. It receives a deeper sync than other supplementary files because §5 Registry and §4 Context Loading Map are derived from disk state. They MUST stay in lockstep with `.agents/skills/` and `package.json`.
...
- **§5 SKILLS + MODES + MCPs REGISTRY** — tables synced from disk:
  - Skills table: one row per directory under `.agents/skills/` (one-line trigger + purpose from each `SKILL.md` description).
  - `### Skill modes`: prose only, pointing at each skill's `## Mode routing` section. Never build a table of modes there: the skill owns the list.
...
|---|---|---|
| Skill and mode names | All targets | `AGENTS.md` says `sync-ai-context`, onboarding still presents `/refresh-ai-memory` |
...
| AGENTS.md | updated | §4 Context Loading Map and §5 Skills table synced with canonical disk state |
...
| docs/core/empezar-aqui.html | updated | skill/mode text nodes and the scripts reference synchronized |
