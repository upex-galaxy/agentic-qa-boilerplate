...
|---|---|---|---|---|---|
| Onboarding del producto | Product onboarding | MTP | once per product | Written once, outside the sprint cycle, when the team enters the product. Not an IQL step because it does not repeat with every story: it is the starting condition of every cycle that follows | `/master-test-plan` (repo: the MTP is the QA Master Test Plan Epic, produced through `/project-context`; see `agentic-qa-core/references/artifact-lifecycle.md` and `agentic-qa-core/references/planning-ladder.md` §1.1) |
