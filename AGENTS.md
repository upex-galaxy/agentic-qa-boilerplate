...
**WORKFLOW SKILL COMPLIANCE**: `shift-left-testing`, `sprint-testing`, `test-documentation`, `test-automation`, `regression-testing`, `framework-development` MUST have `## Subagent Dispatch Strategy` using 7-component briefing, AND close their final stage per `agentic-qa-core/references/session-footer-contract.md`. EXEMPT (reference/utility/generator): `agentic-qa-core`, `agentic-qa-onboard`, `acli`, `xray-cli`, `playwright-cli`, `playwright-best-practices`, `project-discovery`, `project-context`, `sync-ai-context`, `adapt-framework`, `jira-administration`, `git-flow-master`, `session-handoff`.
...
Each row names the committed `.mcp.json` server and, in parentheses, the CAPABILITY skills declare for it (`metadata.requires_capabilities`) and resolve by tool-name suffix, so a user-level server or a claude.ai connector exposing the same tools satisfies the row: canon `agentic-qa-core/references/mcp-capabilities.md`.

| MCP (capability) | Use for | Rule |
|---|---|---|
| Playwright (`browser`) | E2E, UI automation, screenshots | Fallback for `[AUTOMATION_TOOL]` (primary = `/playwright-cli`) |
| OpenAPI (`api-schema`) | API **schema** read-only (endpoint discovery, request/response contracts) | `[API_TOOL]` schema-read leg ONLY. Authenticated execution is `curl`, NOT the MCP: see `agentic-qa-core/references/api-testing-doctrine.md`. |
| DBHub (`db`) | DB queries, data validation | `[DB_TOOL]` primary |
| Context7 (`library-docs`) | Library official docs ("how to use X") | `[DOCS_TOOL]` primary. **MANDATORY** for any library / framework / SDK / API / CLI doc lookup (React, Next, Playwright, Prisma, Tailwind, Express, etc.). PREFER OVER built-in `WebSearch` / `WebFetch`: Context7 returns current versioned docs; built-in web search returns stale blog posts. |
| Tavily (`web-search`) | Community solutions ("how to solve X"), troubleshooting, non-doc web research | `[WEB_SEARCH_TOOL]` primary. **MANDATORY** for any general web search: community fixes, error message lookups, "how to solve X". PREFER OVER built-in `WebSearch` / `WebFetch`: Tavily returns ranked + summarized results; built-in is shallower. |
...
| `[WEB_SEARCH_TOOL]` | General web search, community fixes, troubleshooting, non-doc research | Capability `web-search`: any MCP tool whose name ENDS in `tavily_search` / `tavily_extract` / `tavily_research`, whichever server exposes it (project `.mcp.json` → `mcp__tavily__…`; a claude.ai connector → `mcp__claude_ai_<name>__…`) | built-in `WebSearch` / `WebFetch` ONLY when the user chooses it after the point-of-use STOP |
...
| `orca` | `/orca-orchestration` (in-repo). That skill holds WHEN/WHAT; the stubs in `orchestration.orchestrator_skills` load ALONGSIDE it (~2k tokens the pair), and the DEEP topics stay served by the binary on demand, never copied into the repo |
...
- **Repo-relative paths in `scripts/` go through `scripts/lib/posix-path.ts`** (`toPosix` / `relativePosix`). `relative()` and `join()` emit `\` on Windows, and this repo has shipped that bug three times: into `kata-manifest.json`, into `REGISTRY.md`, and into `lint-vars.ts`'s allowlist compare (downstream issue #26). **Exception: path GUARDS compare in the platform's own `sep`, never normalised**, because normalising a traversal prefix is how a `\` candidate slips past a `/` prefix.
...
Git / PR work → `/git-flow-master` auto-loads. Details in `.agents/skills/git-flow-master/`. **No legacy git-flow doc applies**: the former docs/workflows/git-flow.md (removed 2026-09-24) mandated a `staging` branch and a PR flow that this repo does not run, and `staging` does not exist on `origin`. The live policy is the `git_strategy:` block in `.agents/project.yaml`, enforced by `bun run git:policy verify`.
