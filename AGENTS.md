...
**ROOT CONFIGS AND THE VARIABLES MODULE SHIP IN HALVES.** `tsconfig.base.json` and `eslint.config.base.js` are SYNCED and hold the path aliases and the `cli/` import-closure rule, while `tsconfig.json` and `eslint.config.js` stay project-owned and extend or spread them. `config/variables.core.ts` is the same shape for the variables module: synced resolver and framework blocks, project-owned environment and URL map in `config/variables.ts`. Never hand-edit a `*.base.*` or `*.core.ts` file in a consumer repo — edit the project half.
