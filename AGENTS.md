...
**HOOK: one emitter, three adapters.** `.agents/hooks/personality-reinject.mjs` holds the contract text once. Claude and Codex execute it as a command hook (stdout becomes developer context on both); OpenCode imports the constant from a thin plugin. Contract enforced by `scripts/agent-compatibility-contracts.ts`: no absolute personal paths, no duplicated hook file, OpenCode must mutate `output.system` in place. Codex's adapter carries `commandWindows` for PowerShell and resolves the repo via `git rev-parse --show-toplevel`.
