...
| One Claude + one OpenCode wrapper per alias (`.claude/commands/*.md`, `.opencode/commands/*.md`; one per alias in the merged manifest) | `.agents/compatibility/command-aliases.json` + optional overlay `command-aliases.project.json` | `bun run agents:compat` |
...
The alias list is the manifest itself; `bun run agents:compat:check` validates every wrapper against it. Each entry names its target skill and mode, e.g. `/break-down-tests` → `test-automation` mode `explain` (read-only).
