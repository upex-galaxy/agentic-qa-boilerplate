...
Stage 1 — Planning
    -> For Story: triage risk + Test Analysis + ATP/ATR + TC OUTLINES (names + 1-line precond/expected)
                  ...
                  jira-xray   -> Set-first order (see "Stage 1 Set-first order" below): ① ATP item
                                 find-or-created FROM the {{jira.acceptance_test_plan}} field content
                                 ② CREATE + EXECUTE `Test` issues at executable detail + the Story's
                                 ATS (Test Set) holding ALL of them ③ ATP/ATR test lists DERIVED from
                                 the ATS membership ④ ATR always created WITH the Test Environment.
