...
9. **Traceability check after Stage 1** (Modality jira-xray): run `[TMS_TOOL] trace {TICKET}` and verify the model — the **Story is linked ONLY to its ATP (Test Plan) and ATR (Test Execution)** as "is tested by". **Individual TCs are NOT linked directly to the Story** (avoids noise); they aggregate via ATP/ATR — the **ATP "designs"** the TCs (TC "is designed by" ATP) and the **ATR "executes"** the TCs (TC "is executed by" ATR). So verify: Story↔ATP, Story↔ATR, each TC↔ATP (designed-by), each TC↔ATR (executed-by) — NOT TC↔Story. Full doctrine: `agentic-qa-core/references/traceability-linking.md` + `test-documentation/references/tms-architecture.md`. Bugs: traceability "gaps" for missing TCs are expected and OK.
