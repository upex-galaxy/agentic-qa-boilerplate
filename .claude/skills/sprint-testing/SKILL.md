...
- Stage 3: Author ATR Test Report -> write to the `{{jira.acceptance_test_results}}` field (or fallback comment) via `[ISSUE_TRACKER_TOOL]` -> QA comment via `[ISSUE_TRACKER_TOOL]` -> transition ticket -> run `bun run jira:sync-issues get <KEY> --include-comments` and read the synced `acceptance-test-results.md` in the PBI.
