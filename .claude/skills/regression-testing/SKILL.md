...
CI artifacts (`merged-allure-results-{env}`) are still produced by the workflow and downloaded via `gh run download` as documented in Phase 2. The published GitHub Pages reports are generated by `scripts/ci/publish-allure-pages.ts` with the SAME `allurerc.mjs` and `allure` devDep as local runs — the `/{env}/{suite}/` URL redirects to the latest run's multi-view landing (Awesome + QA Dashboard + Smoke Release Gate), with per-suite trend history and last-10-runs retention.
