...
2. TC names follow the pattern `{US_ID}: TC#: should <expected outcome> [<connector> <condition>] [given <precondition>]`, where `CORE` is the expected outcome (verb + object phrased after `should`, e.g., `grant access`, `reject login`) and `CONDITIONAL` is the optional connector clause (`when …` / `if …`, e.g., `when credentials are valid`, `if password is incorrect`) plus an optional `given …` precondition. The prefix is ALWAYS the User Story ID, in every modality; Test Set membership is never baked into the title. It is also **not** a Jira issue link — membership is Xray-internal, managed through `/xray-cli`, never `acli link create` (see `.agents/jira-required.yaml` → `link_types`). A consequence worth knowing: `bun run jira:sync-issues` talks to the Jira REST API, so it cannot see Test Set membership or Precondition associations at all — only the Xray GraphQL API exposes them.
