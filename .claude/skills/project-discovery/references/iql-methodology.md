...
| Phase 2 — SRS | `architecture.md`, `functional-specs.md`, `non-functional-specs.md` (+ API contract source recorded in `project-config.md` — spec URL, `api/openapi-types.ts`, or Discovery Gap) | Step 5 (TC Documentation) + Step 6 (Automation Candidate Evaluation) | TCs cite FR-N entries; request/response oracles for automation come from `api/openapi-types.ts` (technical) or `business-api-map.md` (business). |
