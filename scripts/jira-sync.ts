 ...
 * (matching a top-level key in `.agents/jira.json`). The actual `customfield_XXXXX`
...
/**
 * Loads `.agents/jira.json` from the repo root. The file is generated by
...
function loadJiraFields(): Record<string, JiraFieldEntry> {
  const path = join(import.meta.dir, '..', '.agents', 'jira.json');
  if (!existsSync(path)) {
    throw new Error(
...
/**
 * Resolves every entry in `SLUG_MAPPING` against `.agents/jira.json` and returns
...
/** Custom field IDs resolved at runtime from `.agents/jira.json` (see SLUG_MAPPING). */
