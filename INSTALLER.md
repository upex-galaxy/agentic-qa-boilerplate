...
- `.env` population — discovers credential placeholders in the MCP config of every selected harness (`${VAR}` in `.mcp.json`, `{file:.auth/opencode/VAR}` in `opencode.jsonc`, generated by `bun run harness:env`; `env_vars` / `bearer_token_env_var` keys in `.codex/config.toml`), then prompts for values not already set
- `bun run harness:env`: derives the Claude Code surface (`env` block of `.claude/settings.local.json`) and the OpenCode surface (`.auth/opencode/<VAR>` files) from `.env`; `bun run setup --variables` reruns it on its own at the end. After filling `.env`: `bun run harness:env`, then restart the agent session (MCP servers read credentials at startup)
- `direnv allow` — optional; exports `.env` into your shell, which only Codex and shell-exported CLI vars (`acli`, `curl`, `bun xray`) need. Never run on its own in a non-interactive setup
...
| ---------------------------------------------------------------------------------- | ----------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Bun**                                                                            | the floor the preflight enforces | `bun run setup:doctor --preflight` (Step 0) | `✗ Preflight failed · Bun X.Y.Z is too old (need >= <min>) · Fix: bun upgrade`                  |
| **`node` (the real binary)**: hooks run `node .agents/hooks/personality-reinject.mjs` | `MIN_NODE_MAJOR` in `cli/doctor.ts` | Scaffolder doctor (`packages/…/doctor.ts`) + `bun run setup:doctor --preflight` (Step 0) | `node >= <min> · not found on PATH — install node >= <min>: https://nodejs.org`                       |
...
`.env` is the single source of credentials, but no harness reads it directly. Claude Code reads the `env` block of `.claude/settings.local.json`; OpenCode reads `.auth/opencode/<VAR>` files via `{file:}`; Codex reads the process environment (`bun run codex` or direnv). `bun run harness:env` derives the first two from `.env`, and `bun run setup:doctor` reports drift. After filling `.env`: run `bun run harness:env`, then restart the agent session (MCP servers read credentials at startup). `bun run setup` finishes with two ways to launch, both of which put MCP env vars (e.g. `ATLASSIAN_API_TOKEN`) into the process environment Codex and the shell CLIs read:
...
All three MCP configs are committed with credential placeholders — `${VAR}` in `.mcp.json` (Claude Code, resolved from the `env` block of `.claude/settings.local.json`), `{file:.auth/opencode/VAR}` in `opencode.jsonc` (OpenCode, one value file per variable), `env_vars` / `bearer_token_env_var` keys in `.codex/config.toml` (Codex, read from the process environment). Real values live in `.env` (gitignored); `bun run harness:env` derives the first two surfaces from it. A fresh clone works for OpenCode before `bun run setup` because `bun install` creates empty `.auth/opencode/*` placeholders (degraded, not broken). If a server returns 401/403 at first call, the matching env var is missing — see `AGENTS.md` Critical Rule #10 (stop, fix `.env`, `bun run harness:env`, restart the agent session).
...
- **Commands.** The slash commands carry no workflow body. Both wrapper sets are generated from `.agents/compatibility/command-aliases.json`; each names a target skill plus a mode and forwards `$ARGUMENTS`. Codex skips the wrapper layer and invokes the skill directly.
...
- **MCPs not loading at all** — run `bun run setup:doctor`: it reports drift between `.env` and the harness surfaces (`.claude/settings.local.json` `env` block for Claude Code, `.auth/opencode/*` for OpenCode); `bun run harness:env` regenerates them. For Codex, confirm you launched via `bun run codex` or that direnv autoload is active (`direnv status` shows your `.envrc` allowed): it reads the process environment, so the bare binary without either path never sees the vars.
