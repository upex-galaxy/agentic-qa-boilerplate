...
describe('seedProjectYamlFromSchema', () => {
  ...
  // host. Every byte of `git_strategy.description` below is this repo's, and
  // the old field-by-field reset left all of it in every scaffolded project.
  const MAINTAINER_YAML = [
    '# Project configuration consumed by AI agents',
    '# Every unfilled field is `null` plus a TODO comment.',
    '',
    'project:',
    '  project_name: null # TODO: Project name',
    '',
    ...
    '  policy:',
    '    direct_push_to_protected: allowed',
    '    require_pr_reviews: 1 # VERIFIED against ruleset 16809531',
    '  meta:',
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'seed-schema-'));
    mkdirSync(join(dir, '.agents'), { recursive: true });
  ...
  // The whole point of the decoupling: the reset used to clear the provenance
  // STAMPS and leave the ruleset id and the admin-bypass prose behind.
  test('no maintainer identity survives the seed', async () => {
    await seedProjectYamlFromSchema(dir);
    const yaml = readYaml();
    expect(yaml).not.toContain('16809531');
    expect(yaml).not.toContain('ProtectPublic');
    expect(yaml).not.toContain('2026-08-21');
    expect(yaml).not.toContain('direct_push_to_protected: allowed');
  });

  // The banner is true of the template and a lie on the copy a project edits.
  ...
  test('the seeded file still starts its content at the first block', async () => {
    await seedProjectYamlFromSchema(dir);
  ...
  test('a template without a schema reports false instead of guessing', async () => {
