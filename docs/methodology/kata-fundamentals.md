...
>
> **Runtime warning specific to this file.** The real runtime of this boilerplate is **TypeScript +
> Playwright + Bun**, with components under `tests/components/` and the `@atc` / `@step` decorators
> in `tests/utils/decorators.ts`. The code samples below are **pytest** (`conftest.py`,
> `import pytest`, `@pytest.fixture`, `pytest_sessionfinish`) and the directory tree names
> `conftest.py` and `pytest.ini` — **neither file exists in this repo, and neither ever did**. Read
...
> **ANTI-PATTERN — do not copy the `summary` line below.** The illustrative script (and its commented TestRail twin) mints a per-run Test Execution titled `KATA Execution - {BUILD_ID}`. That is exactly the shape this repo removed: a brand-new, unparented item per run, outside the ratified `{ACRONYM}: {scope}: {desc}` title grammar, which Xray's import API can never re-parent afterwards (its `info` schema has no parent field). The nuance: a per-run Execution is the RIGHT shape when it is parented (`QA Test Artifacts`), titled in the grammar (`RTR: {scope-id}: Regression Testing`) and plan-linked (`testPlan → RTP`); that is the RTR, one per regression verdict. The anti-pattern is the unparented, untitled, plan-less one the fallback used to mint. The real path imports onto an **existing Execution** whose key is `STP_EXECUTION_KEY` (the RTR of the current run by default; the sprint-close STR, linked to the sprint's STP, when the run is the sprint close), under the `QA Test Artifacts` epic. See `tests/utils/jiraSync.ts` for the implementation that validates the key's issue type and refuses a Test Plan. The block below is kept only as a language-agnostic illustration of the payload shape.
