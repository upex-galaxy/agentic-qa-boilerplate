...
| 4 | ADR file numbering | `ADR-{NNNN}-{slug}.md`, number allocated MANUALLY as `max(NNNN)+1` from the README index (the `bun run adr:next` pre-allocator was NOT built — see delta above) | `.context/ADR/README.md` · `agentic-qa-core/references/adr-doctrine.md` |
| 5 | Env identifiers | `local · qa · staging · production` (lowercase, no abbreviations) | `.agents/project.yaml` `environments` · AGENTS.md §7 |
...
| 8 | Test-execution folders | the EXISTING sync convention: `test-executions/{ATR\|STR\|RETEST}-{KEY}-{slug}.md`, with `TESTEXEC-` / `RETESTEXEC-` kept for non-conforming titles (the `{EXEC-KEY}-{ts}/` folder shape was REJECTED — see delta above) | AGENTS.md §9 PBI tree · `scripts/sync-jira-issues.ts` |
| 9 | Nested defect files | the EXISTING sync convention: one flat file per defect, `defects/DEFECT-{KEY}-{slug}.md` (the nested `bug.md` + `evidence/` + `related-tests/` layout was REJECTED — see delta above) | AGENTS.md §9 · `scripts/sync-jira-issues.ts` |
| 10 | Data factory / types | `DataFactory.ts` · `types.ts` under `tests/data/` (naming documented only; no stub files were scaffolded, and there is no `constants.ts` on disk — see delta above) | `test-automation/references/kata-architecture.md` |
...
## Suggested batching (historical — the record of how the roll-out was sequenced)
