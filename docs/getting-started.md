...
- `/sync-ai-memory` — syncs all AI-critical documents (`CLAUDE.md`, `README.md`, `INSTALLER.md`, `CONTEXT.md`, `docs/*.md`, `docs/onboarding/index.html`) against current `.context/` and `package.json`. Touches FACTS only — never structural sections.
...
| **`/sync-ai-memory`** (command) | Periodically, when `.context/` drifts and AI-critical docs no longer reflect the project state. | Syncs all AI-critical documents (`CLAUDE.md`, `README.md`, `INSTALLER.md`, `CONTEXT.md`, `docs/*.md`, `docs/onboarding/index.html`) against current `.context/` and `package.json`. Touches project-specific FACTS only (Project Identity, Environment URLs, Discovery Progress) — never structural sections. | Periodic |

Mnemonic:

> `agentic-qa-core` installs the **base**, `adapt-framework` wires the **tests**, `sync-ai-memory` syncs the **documentation**.
...
| Sync AI-critical docs after drift | `/sync-ai-memory` |
...
- **`.context/`** = what the AI KNOWS about the target (generated by `/project-discovery`, kept fresh by `/sync-ai-memory` + `/business-*` commands).
