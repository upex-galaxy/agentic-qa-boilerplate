...
It detects gentle-ai (or skips it), installs Engram + 11 SDD skills + 2 universal helpers, configures the 7 canonical MCPs, downloads Playwright browsers, populates `.agents/project.yaml`, and writes a state file so re-runs are idempotent. Then in your agent invoke `/agentic-qa-onboard` for the orientation tour.
